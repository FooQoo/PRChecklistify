...
This file provides instructions and guidelines for the copilot to assist with development within the `pages/side-panel` project. It integrates information from all README files within the project to ensure consistency and adherence to project standards.

## General Guidelines

*   **Language:** Use TypeScript for all new code.
*   **Testing:** Ensure all components and functions are thoroughly tested.
*   **Performance:** Pay attention to performance considerations, especially when dealing with large datasets or complex calculations.
*   **Error Handling:** Always handle errors and loading states gracefully, providing informative feedback to the user.
*   **Documentation:** Document all components and functions with clear and concise comments. Use JSDoc-style comments.
*   **Clean Code:** Keep the codebase clean and well-organized.
*   **Chrome Storage API:** Use Chrome Storage API with caution, as storage is limited. Store API keys securely.
*   **User Experience:** Consider the user experience when designing new features or making changes to existing ones.
*   **Naming Conventions:**  Adhere to the naming conventions outlined below.
*   **Modularity and Reusability:**  When creating new components, consider their reusability and modularity.

## Naming Conventions

*   **Components:** PascalCase (e.g., `SidePanel`, `AppRoutes`, `FileChatModal`). Component file names are written in PascalCase (e.g., `FileChatModal.tsx`).
*   **Files:** PascalCase or camelCase depending on the primary export (e.g., `SidePanel.tsx`, `currentPageAtom.ts`, `aiService.ts`). File names should be descriptive and use PascalCase (e.g., `AppRoutes.tsx`).
*   **CSS Classes:** BEM-like structure (Block-Element-Modifier) where appropriate, using Tailwind CSS utility classes when possible.
*   **Atoms:** camelCase ending with "Atom" (e.g., `currentPageAtom`, `generatingAtom`). Atom names should be descriptive and clearly indicate the state they manage.
*   **Props Interfaces:** Props interfaces are named after the component, suffixed with `Props` (e.g., `FileChatModalProps`, `NavigationProviderProps`).
*   **Context Variables and Functions:** Context variables and functions should use camelCase (e.g., `navigateToPr`).
*   **Interface Names:** Interface names should use PascalCase (e.g., `NavigationContextType`, `CurrentPage`). Interfaces are named with a capitalized first letter (PascalCase) and typically end with `Data`, `Item`, or `Result` to clearly indicate their purpose.  For example, `PRData`, `ChecklistItem`, `PRAnalysisResult`.
*   **Hook Names:** Hook names should start with `use` followed by a descriptive name of what the hook does, e.g., `usePRData`, `useGithubTokenAtom`, `useMyCustomHook`.
*   **Setter and Remover Functions:** Setter and remover functions for atom values should use `set[Noun]AndStorage` and `clear[Noun]`, respectively, to denote they also interact with persistent storage.
*   **Route Paths:** Route paths should be lowercase and use hyphens to separate words (e.g., `/github-token-setup`).
*   **Variables and Constants:** Variables and constants should use camelCase (e.g., `router`, `routes`). Constants are in UPPER_SNAKE_CASE (e.g., `STORAGE_KEY`, `MAX_CACHE_SIZE`).
*   **Functions in services:** Function names are in camelCase (e.g., `fetchPullRequest`, `generateAnalysis`).
*   **Class names in services:** Class names are in PascalCase (e.g., `GithubClient`, `OpenAIClient`).

## Design Policies

*   **Component-Based UI:** Structure the UI into reusable React components.
*   **Functional Components:** Primarily use functional components with hooks.
*   **Modular and Reusable Components:** Design components to be modular and reusable.
*   **Consistent UI:** UI elements follow a consistent design language to ensure a cohesive user experience.
*   **Accessibility and Responsiveness:** Components prioritize accessibility and responsiveness.
*   **Error Boundaries:** Wrap components with error boundaries for robust error handling using `withErrorBoundary`.
*   **Suspense:** Manage loading states using React Suspense or `withSuspense`.
*   **State Management:** Utilize Jotai for state management. Keep atoms focused on single responsibilities.
*   **Centralized Navigation:** Centralize navigation logic within the context to maintain consistency.
*   **Clear and Intuitive Routing:** The routing structure should be clear and intuitive for users.
*   **Single Responsibility Principle (Services):** Each service should be responsible for a specific set of tasks.
...
*   **React Router:** Standard library for routing in React applications. Specifically, `createMemoryRouter` and `RouterProvider` are used.
...
*   **Use TypeScript:** Use TypeScript for all new code.
*   **Follow Naming Conventions:** Adhere to the naming conventions outlined above.
*   **Error Handling:** Implement proper error handling using `withErrorBoundary` or try/catch blocks.
*   **Loading States:** Manage loading states with `withSuspense` or appropriate loading indicators. Indicate loading states while performing asynchronous operations.
*   **State Management:** Utilize Jotai for state management. Keep atoms focused.  Use the `useAtom` hook to read and write atom values. Define types for atom values.  Minimize dependencies between atoms.
*   **Modular Components:** Write modular and reusable components.
*   **Tailwind CSS:** Use Tailwind CSS utility classes for styling where applicable.
*   **Code Comments:** Document all components and functions with clear and concise comments.
*   **React Context Usage:** Always use the `useNavigation` hook to access the navigation context within functional components. Ensure that components that use the `useNavigation` hook are wrapped within the `NavigationProvider`.
*   **Navigation Logic:** Centralize navigation logic within the `NavigationContext` to maintain consistency. When adding new navigation functions, update the `NavigationContextType` interface and the `contextValue` object.
*   **Hook Naming:** Always start custom hook names with `use`.
*   **Storage Interaction:** When interacting with storage, use the provided storage services (e.g., `@extension/storage`). When managing API keys or tokens, use secure storage mechanisms and avoid exposing them directly in the UI.
*   **Jotai with Storage Synchronization:** Use the Jotai atom pattern with storage synchronization for global persisted states. When synchronizing Jotai atoms with `localStorage`, handle the case when component unmounts to avoid memory leaks by using a `mounted` flag in the `useEffect`.
*   **API Key Management:** API keys must be stored securely using the Chrome Storage API and should never be hardcoded.
*   **Data Validation (Services):** Validate data received from external APIs to ensure it conforms to the expected schema.
*   **Asynchronous Operations (Services):** Use `async/await` for asynchronous operations to improve code readability.
*   **Singletons (Services):** For classes like `GithubClient`, use the singleton pattern.
...
| File Name              | Role                                                              | Logic and Functions                                                                                                                                                                                                                                                                                                                                                                                                                    | Names of other files used (Dependencies)                                       |
| ---------------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `DefaultView.tsx`      | Default view for entering and navigating to GitHub PRs.            | - `useState`: Manages the input URL, validation status, recent PRs, and visibility of all recent PRs.- `useEffect`: Loads recent PRs from local storage on component mount.- `validateUrl`: Validates the entered URL using `isGitHubPRPage`.- `handleUrlChange`: Updates the URL state and validates it.- `handleGoToPR`: Navigates to the PR using `navigateToPr` if the URL is valid.- `handleRecentPRClick`: Navigates to a PR from history using `navigateToPrFromHistory`.- Logic to display either the first 5 recent PRs or all, with a button to toggle. | `../context/NavigationContext`, `../utils/prUtils`                               |
| `GitHubPRView.tsx`     | View for displaying GitHub PR data and analysis.                    | - `useParams`: Extracts the `owner`, `repo`, and `prNumber` from the URL.- `usePRData` hook: Fetches PR data, loading state, analysis results, and approval percentage.- `useEffect`: Triggers confetti effect when PR is marked as complete (100% approval).- Renders loading state, error state, or PR data based on the current state.- Displays review progress with approved files count and estimated review time. | `../hooks/usePRData`, `../utils/prUtils`, `../components/PRAnalysis`, `react-rewards`, `react-router-dom` |
...
| `OpenAiTokenSetupView.tsx` | View for setting up the OpenAI API key.                         | - `useState`: Manages the API key input, loading state, selected provider, and error messages.- `handleSubmit`: Handles form submission, validates the API key using provider specific validation, and saves it to storage.- A `getMaskedApiKey` function provides a masked display of the API key for privacy.-  Manages selection of provider (OpenAI, Gemini) and corresponding API Key input.     | `../context/NavigationContext`, `@extension/i18n`, `../hooks/useOpenaiKeyAtom`, `@src/hooks/useGeminiKeyAtom` |
| `SettingsView.tsx`       | View for managing extension settings (language, API keys, etc.). | - `useState`: Manages the language, OpenAI API endpoint, recent PRs, toast visibility, and model client type (OpenAI or Gemini).- `useEffect`: Loads settings from storage on component mount.- `handleLanguageChange`: Handles language selection and saves it to storage.- `handleModelClientTypeChange`: Handles selection of the AI provider and saves it to storage.- Displays settings forms for GitHub integration, OpenAI API key, and language preference. Displays toast messages for user feedback. | `../context/NavigationContext`, `../components/OpenAIKeySettings`, `../components/GitHubIntegrationSettings`, `../components/GeminiKeySettings`, `../services/modelClient`, `@extension/storage`, `@extension/i18n`, `../components/Toast` |
